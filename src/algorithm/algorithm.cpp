...
namespace emoc
{
	...
	Algorithm::Algorithm(Py_Global *global, Problem *problem) : g_GlobalSettings(nullptr),
																problem_(nullptr),
																real_popnum_(0),
																runtime_(0.0)
	{
		g_GlobalSettings = global;
		problem_ = problem;
	...
	void Algorithm::PrintResult()
	{
		...
		int dec_num = g_GlobalSettings->dec_num_;
		if (obj_num > 1)
		{
			int pf_size = 0;
			double **pf_data = LoadPFData(pf_size, obj_num, problem_->problem_name_);
			HVCalculator hv_calculator(obj_num, real_popnum_);
			double igd = CalculateIGD(g_GlobalSettings->parent_population_.data(), real_popnum_, obj_num, pf_data, pf_size);
			double hv = hv_calculator.Calculate(g_GlobalSettings->parent_population_.data(), real_popnum_, obj_num, pf_data, pf_size);
			double gd = CalculateGD(g_GlobalSettings->parent_population_.data(), real_popnum_, obj_num, pf_data, pf_size);
			// double spacing = CalculateSpacing(g->parent_population_.data(), real_popnum_, obj_num);
			// double igdplus = CalculateIGDPlus(g->parent_population_.data(), real_popnum_, obj_num, pf_data, pf_size);
			// double gdplus = CalculateGDPlus(g->parent_population_.data(), real_popnum_, obj_num, pf_data, pf_size);

			std::cout << "******Finish Optimization******" << std::endl;
			std::cout << "time: " << GetRuntime() << "\t igd: " << igd << "\t hv: " << hv << "\t gd:" << gd << std::endl;
		}
	}
	...
	bool Algorithm::IsTermination()
	{
		// record runtime
		end_ = clock();
		if (g_GlobalSettings->iteration_num_ >= 1)
			runtime_ += (double)(end_ - start_) / CLOCKS_PER_SEC;
	...
	void Algorithm::TrackPopulation()
	{
		// record population
		g_GlobalSettings->RecordPop(real_popnum_, runtime_);
	...
	// 	char plot_cmd[1024];
	// 	if (problem_->encoding_ == Problem::BINARY)
	// 	{
	...
	// 		// }
	// 	}
	// 	else if (problem_->encoding_ == Problem::REAL)
	// 	{
