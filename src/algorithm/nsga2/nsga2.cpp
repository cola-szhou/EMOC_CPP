...
namespace emoc
{
	...
	NSGA2::NSGA2(Py_Global *global, Problem *problem, double crossover_prob, double eta_c, double mutation_prob, double eta_m) : Algorithm(global, problem)
	{
		...
		mutation_para_.index1 = eta_m;

		// set crossover parameter
		cross_para_.pro = crossover_prob;
		cross_para_.index1 = eta_c;
	}
	...
	void NSGA2::Initialization()
	{
		...
		g_GlobalSettings->InitializePopulation(g_GlobalSettings->parent_population_.data(), g_GlobalSettings->population_num_, problem_);
